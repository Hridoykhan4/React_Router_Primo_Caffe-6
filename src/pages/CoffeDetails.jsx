...
import { addFavorite, getStoredFavorite } from "../utils/indexLocal";
const CoffeDetails = () => {
  ...
  useEffect(() => {
    const findMatchedCoffee = allCoffeesData.find(
      (coffee) => coffee.id === parseInt(currentId)
    );

    const favorites = getStoredFavorite();
    const isExist = favorites.find(
      (coffee) => coffee.id == findMatchedCoffee.id
    );
    if (isExist) {
      setIsFavorite(true);
    }

    setCoffee(findMatchedCoffee);
  }, [allCoffeesData, currentId]);
